...
import {
  LayoutDashboard, Building2, GraduationCap, Users, UserCog, ShieldCheck,
  CalendarDays, Images, FileBarChart, ScrollText, Settings, User, LogOut,
  Menu, X, Moon, MessageSquare,
} from "lucide-react";
...
const NAV = [
  { section: "Utama", items: [{ to: "/admin/dashboard", label: "Dashboard", icon: LayoutDashboard }] },
  {
    section: "Master Data", items: [
      ...
      { to: "/admin/users", label: "Manajemen User", icon: ShieldCheck, super: true },
    ],
  },
  {
  ...
  {
    section: "Laporan & Sistem", items: [
      { to: "/admin/laporan", label: "Laporan & Export", icon: FileBarChart },
      { to: "/admin/audit", label: "Audit Log", icon: ScrollText },
      { to: "/admin/pengaturan", label: "Pengaturan", icon: Settings, super: true },
    ...
    <div className="flex flex-col h-full">
      <div className="px-5 py-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-gold/90 flex items-center justify-center shrink-0">
            <Moon className="h-5 w-5 text-emerald-900" />
          </div>
          <div className="min-w-0">
            <p className="text-white font-display font-bold leading-tight text-sm">Raudhatul Jannah</p>
            <p className="text-white/60 text-xs">Portal Admin Majelis</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto scrollbar-thin px-3 py-4 space-y-5">
        {NAV.map((grp) => (
          <div key={grp.section}>
            <p className="px-3 mb-1.5 text-[10px] uppercase tracking-wider text-white/40 font-semibold">{grp.section}</p>
            <div className="space-y-0.5">
              {grp.items.filter((i) => !i.super || isSuper).map((item) => (
                <NavLink key={item.to} to={item.to} onClick={() => setMobileOpen(false)}
                  data-testid={`nav-${item.to.split("/").pop()}`}
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                      isActive ? "bg-gold text-emerald-900" : "text-white/75 hover:bg-white/10 hover:text-white"
                    }`}>
    ...
    <div className="min-h-screen bg-[#FAFAFA]">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 glass-nav h-16 flex items-center justify-between px-4 sm:px-6">
          <button className="lg:hidden p-2 -ml-2" onClick={() => setMobileOpen(true)} data-testid="mobile-menu-btn">
            {mobileOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
          </button>
          <div className="hidden lg:block" />
          <div className="flex items-center gap-3">
            <a href="/" target="_blank" rel="noreferrer"
              className="text-sm text-primary font-medium hover:underline">Lihat Situs Publik</a>
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button variant="outline" size="sm" className="rounded-xl gap-2" data-testid="logout-btn">
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>Sesi Anda akan diakhiri dengan aman.</AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel className="rounded-xl">Batal</AlertDialogCancel>
                  <AlertDialogAction onClick={handleLogout} className="rounded-xl" data-testid="confirm-logout">
                    Keluar
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
