        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-24 sm:py-32 grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial="hidden" animate="show" variants={fadeUp}>
            <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-tight tracking-tight">
              ...
              <span className="text-gold">Nurul Islam wa Iman</span>
            </h1>
            <p className="mt-6 text-white/80 text-base sm:text-lg leading-relaxed max-w-xl">
              Menaungi kebersamaan spiritual jamaah melalui dzikir, sholawat,
              dan pembinaan ilmu ma'rifatullah wa ma'rifaturrosul di seluruh
              cabang.
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
              <Link to="/profil">
                <Button
                  size="lg"
                  className="rounded-xl bg-gold text-emerald-900 hover:bg-gold/90 gap-2 h-12 px-6"
                  data-testid="hero-profil-btn"
                >
              ...
              <Link to="/cabang">
                <Button
                  size="lg"
                  variant="outline"
                  className="rounded-xl h-12 px-6 border-white/40 bg-white/5 text-white hover:bg-white/15 hover:text-white gap-2"
                  data-testid="hero-cabang-btn"
                >
          ...
          >
            <div className="rounded-[24px] overflow-hidden border-4 border-gold/30 shadow-2xl">
              <img
                src="https://images.unsplash.com/photo-1766166793579-4833898111a5?crop=entropy&cs=srgb&fm=jpg&q=85&w=900"
                alt="Interior masjid dengan ornamen emas"
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {STAT_META.map((s, i) => (
            ...
            >
              ...
              <p className="text-3xl font-bold text-charcoal font-display">
                {stats[s.key] ?? 0}
              </p>
            ...
            {announcements.map((a, i) => (
              ...
              >
                ...
                <div>
                  <Badge
                    variant="secondary"
                    className="rounded-full mb-2 text-xs"
                  >
                    {a.kategori}
                  </Badge>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-24">
        <div className="grid md:grid-cols-3 gap-6">
          {[
            {
              to: "/pendiri",
              title: "Pendiri & Penerus",
              desc: "Mengenal masyaikh pengamanah ilmu majelis.",
              icon: Sparkles,
            },
            {
              to: "/cabang",
              title: "Cabang & Peta",
              desc: "Temukan cabang majelis terdekat dari Anda.",
              icon: MapPin,
            },
            {
              to: "/galeri",
              title: "Galeri Kegiatan",
              desc: "Dokumentasi kegiatan dzikir & sholawat.",
              icon: Building2,
            },
          ].map((q) => (
            <Link
              key={q.to}
              to={q.to}
              className="premium-card p-8 group hover:-translate-y-1 transition-transform"
              data-testid={`quicklink-${q.to.slice(1)}`}
            >
