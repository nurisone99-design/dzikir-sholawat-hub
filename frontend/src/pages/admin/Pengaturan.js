...
export default function Pengaturan() {
  ...
  const backup = async () => {
    try {
      ...
      a.href = url;
      a.download = `backup-raudhatuljannah-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
      URL.revokeObjectURL(url);
      toast.success("Backup berhasil diunduh", { id: "bk" });
    } catch (e) {
      toast.error("Gagal membuat backup", { id: "bk" });
    }
  };
  ...
  const restore = async (file) => {
    ...
    try {
      ...
      toast.error("Gagal memulihkan: file tidak valid");
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Identitas yayasan, notifikasi & backup data
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="identitas">
        <TabsList className="rounded-xl">
          <TabsTrigger value="identitas" className="rounded-lg gap-2">
            <Building2 className="h-4 w-4" /> Identitas
          </TabsTrigger>
          <TabsTrigger value="notif" className="rounded-lg gap-2">
            <Bell className="h-4 w-4" /> Notifikasi & WA
          </TabsTrigger>
          {isSuper && (
            <TabsTrigger value="backup" className="rounded-lg gap-2">
          ...
          <div className="premium-card p-8 max-w-3xl space-y-5">
            <div className="grid sm:grid-cols-2 gap-5">
              <div className="sm:col-span-2">
                <Label className="mb-1.5 block text-sm">Nama Yayasan</Label>
                <Input
                  value={form.nama || ""}
                  onChange={(e) => upd("nama", e.target.value)}
                  className="rounded-xl"
                  data-testid="setting-nama"
                />
              </div>
              <div className="sm:col-span-2">
                <Label className="mb-1.5 block text-sm">Nama Majelis</Label>
                <Input
                  value={form.nama_majelis || ""}
                  onChange={(e) => upd("nama_majelis", e.target.value)}
                  className="rounded-xl"
                />
              </div>
              <div className="sm:col-span-2">
                <Label className="mb-1.5 block text-sm">Alamat</Label>
                <Input
                  value={form.alamat || ""}
              ...
              <div>
                <Label className="mb-1.5 block text-sm">Telepon</Label>
                <Input
                  value={form.telepon || ""}
                  onChange={(e) => upd("telepon", e.target.value)}
                  className="rounded-xl"
                />
              </div>
              <div>
              ...
              <div>
                <Label className="mb-1.5 block text-sm">YouTube</Label>
                <Input
                  value={form.youtube || ""}
          ...
          <div className="premium-card p-8 max-w-3xl space-y-5">
            <div className="flex items-center justify-between rounded-xl bg-secondary/50 px-4 py-3">
              ...
              <Switch
                ...
                onCheckedChange={(v) => upd("notif_email", v)}
              />
            </div>
            <div>
              <Label className="mb-1.5 block text-sm">
                No. WhatsApp Yayasan (Click-to-Chat)
              </Label>
              <Input
                value={form.whatsapp || ""}
            ...
            <div>
              <Label className="mb-1.5 block text-sm flex items-center gap-1.5">
                <KeyRound className="h-3.5 w-3.5" /> WhatsApp API Key (opsional)
              </Label>
              <Input
                value={form.wa_api_key || ""}
                onChange={(e) => upd("wa_api_key", e.target.value)}
                placeholder="Kosongkan jika menggunakan wa.me"
                className="rounded-xl"
              />
              <p className="text-xs text-muted-foreground mt-1.5">
                Broadcast saat ini menggunakan WhatsApp Click-to-Chat (wa.me) —
            ...
            <div className="grid md:grid-cols-2 gap-5 max-w-3xl">
              <div className="premium-card p-8">
                ...
                <p className="text-sm text-muted-foreground mt-1.5 mb-5">
                  Unduh seluruh data sistem dalam format JSON.
                </p>
                <Button
                  onClick={backup}
              ...
              <div className="premium-card p-8">
                ...
                <p className="text-sm text-muted-foreground mt-1.5 mb-5">
                  Pulihkan data dari file backup JSON. Data lama akan diganti.
                </p>
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button
                      variant="outline"
                      className="rounded-xl gap-2 w-full border-gold text-gold hover:bg-gold/5"
                      data-testid="restore-btn"
                    >
                      <Upload className="h-4 w-4" /> Pilih File Restore
                    </Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Seluruh data saat ini akan digantikan oleh isi file
                        backup. Tindakan ini tidak dapat dibatalkan.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
